...
from models.hierarchical import TabFormerEmbeddings, TabStaticFormerEmbeddings, TabFormerFCEmbeddings
from models.tabformer_bert import TabFormerBertForMaskedLM, TabFormerBertConfig, TabFormerBertForSequenceClassification, \
    TabFormerBertForPretraining
from models.tabformer_gpt2 import TabFormerGPT2LMHeadModel
from models.tabformer_static_bert import TabStaticTimePosFormerBertForMaskedLM, \
    TabStaticTimePosFormerBertForPretraining, TabStaticTimePosFormerBertForClassification, TabStaticFormerBertConfig, \
    MLP_Classification
from models.classifier import TabRawForClassification, TabEmbeddingsForClassification, \
    TabEmbeddingsLSTMForClassification, TabEmbeddingsStaticLSTMForClassification
...
class TabFormerHierarchicalLM_fc(TabFormerPreTrainedModel):
    base_model_prefix = "bert"
    ...
    def __init__(self, config, vocab):
        ...
        self.tab_embeddings = TabFormerFCEmbeddings(self.config)
        self.tb_model = TabFormerBertForMaskedLM(self.config, vocab)
...
class TabStaticFormerHierarchicalLM_fc(TabFormerPreTrainedModel):
    base_model_prefix = "bert"
    ...
    def __init__(self, config, vocab):
        ...
        self.static_tab_embeddings = TabFormerFCEmbeddings(self.config)
        self.tb_model = TabStaticTimePosFormerBertForMaskedLM(
            self.config, vocab)
...
class TabStaticFormerHierarchicalForPretraining_fc(TabStaticFormerHierarchicalLM):
    base_model_prefix = "bert"
...
class Transformer_MLP_Classification(TabFormerPreTrainedModel):
    ...
    def __init__(self, config, vocab):
        ...
        self.config = config
        self.tab_embeddings = TabFormerEmbeddings(self.config)
        self.tb_model = MLP_Classification(self.config, vocab)

    def forward(self, input_ids, labels, **input_args):
        inputs_embeds = self.tab_embeddings(input_ids)
        return self.tb_model(inputs_embeds=inputs_embeds, labels=labels, **input_args)
...
class TabFormerHierarchicalForClassification_fc(TabFormerPreTrainedModel):
    base_model_prefix = "bert"
...
class TabFormerBertLM:
    def __init__(self, special_tokens, vocab, field_ce=False, flatten=False, ncols=None, field_hidden_size=768,
        ...
        self.tokenizer = BertTokenizer(vocab_file,
                                       do_lower_case=False,
                                       **special_tokens)
        self.model = self.get_model(field_ce, flatten, fc)

    def get_model(self, field_ce, flatten, fc):

        if fc:
            model = TabFormerHierarchicalLM_fc(self.config, self.vocab)
        elif flatten and not field_ce:
...
class TabStaticFormerBertLM:
    def __init__(self, special_tokens, vocab, field_ce=False, flatten=False, ncols=None, static_ncols=None,
    ...
    def get_model(self, field_ce, flatten):
        ...
        if self.fc:
            model = TabStaticFormerHierarchicalLM_fc(self.config, self.vocab)
        elif flatten and not field_ce:
            # flattened vanilla BERT
            model = BertForMaskedLM(self.config)
        elif flatten and field_ce:
...
class TabStatic_Transformer_MLP_Classification:
    def __init__(self, special_tokens, vocab, field_ce=False, flatten=False, ncols=None, static_ncols=None,
                 field_hidden_size=768,
                 seq_len=0, num_labels=2, time_pos_type=None, pretrained_dir=None, problem_type=None, fc=False):
        ...
        print("---- class TabStatic_Transformer_MLP_Classification:, vocab_file", vocab_file)
        ...
        self.tokenizer = BertTokenizer(vocab_file,
                                       do_lower_case=False,
                                       **special_tokens)
        ## lxw: here self.tokenizer seems never used? Actually, it is used later in data_collator = MaskingLabelTransDataCollatorForLanguageModeling(tokenizer=model.tokenizer, mlm=False)

        self.model = self.get_model(field_ce, flatten)

    def get_model(self, field_ce, flatten):
        if self.pretrained_dir is not None:
            model = Transformer_MLP_Classification.from_pretrained(
                self.pretrained_dir, config=self.config, vocab=self.vocab)
        else:
            model = Transformer_MLP_Classification(self.config, self.vocab)
        return model
...
class TabStaticFormerBert:
    def __init__(self, special_tokens, vocab, field_ce=False, flatten=False, ncols=None, static_ncols=None,
                 field_hidden_size=768, num_labels=2,
                 seq_len=10, pretrained_dir=None, problem_type=None, time_pos_type=None):
...
class TabFormerBertForClassification:
    def __init__(self, special_tokens, vocab, field_ce=False, flatten=False, ncols=None, field_hidden_size=768,
                 num_labels=2,
                 seq_len=10, pretrained_dir=None, problem_type=None, time_pos_type=None, fc=False):
    ...
    def get_model(self, field_ce, flatten):
        ...
        if self.fc:
            # fullyconnected field CE BERT
            if self.pretrained_dir is not None:
                model = TabFormerHierarchicalForClassification_fc.from_pretrained(
                    self.pretrained_dir, config=self.config, vocab=self.vocab)
            else:
                model = TabFormerHierarchicalForClassification_fc(
                    self.config, self.vocab)
