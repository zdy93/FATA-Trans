...
class AmazonWithTimePosAndStaticSplitDataset(AmazonBasicDataset):
    def __init__(self,
                 cls_task,
                 reviewer_ids=None,
                 seq_len=10,
    ...
    def reviewer_level_split_data(self):
        ...
        if self.reviewer_level_cached and path.isfile(fname):
            ...
            for ix in tqdm.tqdm(range(len(start_idx_list))):
                ...
                reviewer_trans_static, reviewer_trans, reviewer_label, reviewer_time = [], [], [], []
                ...
                for idx, row in reviewer_data.iterrows():
                    row = list(row)
                    reviewer_trans.extend(row[skip_idx + 1:-5])
                    reviewer_label.append(row[skip_idx + 1])
                    reviewer_time.append(row[skip_idx])
                ...
                trans_time.append(reviewer_time)
                if self.get_rids and bks_exist:
    ...
    def prepare_samples(self):
        ...
        if self.get_rids:
            trans_data, trans_labels, trans_rids, trans_time, columns_names, static_columns = self.reviewer_level_split_data()
        else:
            trans_data, trans_labels, trans_time, columns_names, static_columns = self.reviewer_level_split_data()
        ...
        for reviewer_idx in tqdm.tqdm(range(len(trans_data))):
            ...
            reviewer_time = trans_time[reviewer_idx]
            if self.get_rids:
            ...
            if self.pad_seq_first:
                for jdx in range(0 - self.seq_len + 1, min(0, len(reviewer_row_ids) - self.seq_len + 1), self.trans_stride):
                    ...
                    time_tail = reviewer_time[0:(jdx + self.seq_len)]
            ...
            for jdx in range(0, len(reviewer_row_ids) - self.seq_len + 1, self.trans_stride):
                ...
                self.data.append(ids)
                self.static_data.append(reviewer_static_ids)
                time = reviewer_time[jdx:(jdx + self.seq_len)]
