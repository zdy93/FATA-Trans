...
class AmazonPreloadDataset(Dataset):
    """
    This class is for preloading amazon data only
    """
    def __init__(self,
                 reviewer_ids=None,
                 num_bins=10,
    ...
    def init_vocab(self):
        ...
        drop_columns = ['rownumber', 'reviewerID', 'Timestamp']
        dynamic_columns = ['overall', 'asin', 'reviewTimeDayOfWeek', 'reviewTimeHour', 'reviewLen', 'asin_rating']
    ...
    @staticmethod
    def timeEncoder(reviewTime):
        # return number of days
        return (pd.to_datetime(reviewTime) - pd.to_datetime(0)).dt.days

    @staticmethod
    def nanNone(X):
        # empty reviewlen will be 0
    ...
    def encode_data(self):
        ...
        data['reviewLen'] = self.nanNone(data['reviewLen'])

        log.info("timestamp fit transform")
        timestamp = self.timeEncoder(data['reviewTime'])
        data['Timestamp'] = timestamp
        data['timeFeature'] = timestamp

        sub_columns = ['overall', 'asin', 'reviewTimeDayOfWeek', 'reviewTimeHour', 'pos_reviewer']
        ...
        for col_name in tqdm.tqdm(sub_columns):
            ...
            # top_item_1, top_item_2 and top_item_3 should use the asin encoder
            if col_name in ['top_item_1', 'top_item_2', 'top_item_3']:
                encoder_name = 'asin'
            else:
            ...
            if self.encoder_fit.get(encoder_name) is not None:
                ...
                assert not (encoder_name == 'asin' and encoder_name != col_name)
        ...
        amt_columns = ['timeFeature', 'reviewLen', 'avg_rating', 'low_per', 'high_per', 'asin_rating']
        ...
        if self.columns_to_select:
            ...
            # keep time info
            columns_to_select = ['rownumber', 'reviewerID', 'Timestamp', 'overall',
                                 'asin', 'reviewTimeDayOfWeek', 'reviewTimeHour', 'reviewLen', 'asin_rating',
                                 'timeFeature', 'avg_rating', 'low_per', 'high_per', 'pos_reviewer']
